...
const registrarMovimiento = (idProducto, tipo, cantidad, venta) => {
    const sql = 'INSERT INTO Movimiento (idProducto, tipo, cantidad, venta, movFechaHora) VALUES (?, ?, ?, ?, ?)';
   
    const moment = require('moment');
    const movFechaHora = moment().format('YYYY-MM-DD HH:mm:ss');

     // Obtener la fecha y hora actual en formato ISO
    db.run(sql, [idProducto, tipo, cantidad, venta, movFechaHora], (err) => {
        if (err) {
...
app.delete('/inventory/:idProducto', (req, res) => {
    ...
    db.run('DELETE FROM Producto WHERE idProducto = ?', [idProducto], (err) => {
        ...
        registrarMovimiento(idProducto, 'baja', 0, 0);
    });
});
...
app.post('/products', (req, res) => {
    ...
    db.run('INSERT INTO Producto (nombre, precioVenta, categoría, cantidadEnStock) VALUES (?, ?, ?, ?)', [nombre, precioVenta, categoría, cantidadEnStock], (err) => {
        ...
        registrarMovimiento(idProducto, 'alta', cantidadEnStock, 0);
    });
});
